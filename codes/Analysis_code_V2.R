...
select_Table4_temp<- tbl_regression(select_Table4_three_way_interactions, tidy_fun = broom.mixed::tidy,
                                     exponentiate = TRUE,intercept = TRUE,
                                     label = list(risk.status ~ 'Risk Status',
                                                  age.in.months ~ 'Scaled age')) %>%
  bold_p() %>%
...
select_Table4<- tbl_stack(list(select_Table4_temp[[1]],select_Table4_temp[[2]]),
                           group_header = c('Fixed effects',
                                            'Random effects')) %>%
  as_gt() %>%
  gt::tab_style(style = gt::cell_text(weight = 'bold'),
...
gt::gtsave(select_Table4, filename = './results/select_Table4_three_way_interactions.docx', expand = 10)
...
Table5_reduced<- glmer(formula = ia_loc~ (1|Participant),
                       data = Table5_input,
                       family = 'binomial',
                       control = glmerControl(optimizer = 'bobyqa'))

Table5_three_way_interactions<- glmer(formula = ia_loc~ risk.status+
                                        age.in.months+
                                        synch_use+
                                        risk.status*age.in.months+
                                        risk.status*synch_use+
...
dredge_Table5_three_way_interactions<- dredge(Table5_three_way_interactions, trace = TRUE)

options(na.action = "na.omit")

######################Selected model fit##################################
selected_vars_Table5_three_way_interactions<- dredge_Table5_three_way_interactions %>%
  as.data.frame(.) %>%
  filter(delta < 2) %>%
  select(-c(1, (ncol(.)-4): ncol(.))) %>%
  mutate_all(funs(ifelse(is.na(.), 0, 1))) %>%
...
colnames(selected_vars_Table5_three_way_interactions)
select_Table5_three_way_interactions<- glmer(formula = as.formula(
  paste('ia_loc~', 
        gsub(pattern = ':', 
             replacement = '*', 
             x = paste(paste0(colnames(selected_vars_Table5_three_way_interactions), 
                              collapse = '+'), 
                       '(1|Participant)', sep = '+')))),
  data = Table5_input,
  family = 'binomial',
  control = glmerControl(optimizer = 'bobyqa'))
summary(select_Table5_three_way_interactions)

aov_res<- anova(Table5_reduced, select_Table5_three_way_interactions, method = 'LRT')
chi_sq<- aov_res$Chisq[2]
perf<- performance::performance(select_Table5_three_way_interactions)
r2_cond<- perf$R2_conditional
r2_marginal<- perf$R2_marginal

select_Table5_temp<- tbl_regression(select_Table5_three_way_interactions, tidy_fun = broom.mixed::tidy,
                                    exponentiate = TRUE,intercept = TRUE,
                                    label = list(risk.status ~ 'Risk Status',
                                                 age.in.months ~ 'Scaled age',
                                                 synch_use ~ 'Synchrony')) %>%
...
select_Table5<- tbl_stack(list(select_Table5_temp[[1]],select_Table5_temp[[2]]),
                          group_header = c('Fixed effects',
                                           'Random effects')) %>%
  as_gt() %>%
  gt::tab_style(style = gt::cell_text(weight = 'bold'),
...
gt::gtsave(select_Table5, filename = './results/select_Table5_three_way_interactions.docx', expand = 10)
